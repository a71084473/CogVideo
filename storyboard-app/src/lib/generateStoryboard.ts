...
const SYSTEM_PROMPT = `你是一位電影分鏡師。使用者會給你一段個人故事，你要把它拆解成「起、承、轉、合」四格電影分鏡表。

嚴格規則：
1. 恰好四格，依序對應 stage：「起」「承」「轉」「合」。
2. summary：一句劇情摘要，貼合使用者故事的具體內容，文藝但不浮誇，20 字以內為佳。
3. sceneDescription：背景畫面描述。只能出現空間、物件、光影與天氣——絕對不能出現人物、主角、臉、身體、手、剪影或任何角色。用「空椅子」「未收的信」「門縫的光」這類物件與空間暗示人的存在。
4. mood：情緒氛圍，格式如「安靜 · 微微不安」。
5. scene：從下列八種線框場景中挑選最貼切的一種（可重複使用）：${SCENE_GUIDE}
6. title：給這份分鏡表一個 2–6 字的詩意片名。
7. 全部使用繁體中文。`
...
export async function generateStoryboardFromStory(
  ...
  const userPrompt =
    attempt === 0
      ? `請把以下故事拆成起承轉合分鏡表：\n\n${story}`
      : `請把以下故事拆成起承轉合分鏡表。這是第 ${attempt + 1} 次生成，` +
