...
- [ ] **草稿頁除罪化**：`ChatModal` 草稿上方加一句「中途看的是你願不願意學，不是你懂多少。」
      → 解「新手怕被打槍」（persona P1）。
- [ ] **送出隱私說明**：`ChatModal` 草稿旁標注「這段摘要只寄給這位中途，不會公開、不進相簿。」
      → 解「隱私敏感者怕留底」（persona P3）。
- [ ] **前移『試相處』**：`Quiz` 結果頁 CTA 旁加一行「先見面、可試相處，不用一次決定。」
      → 解「有退養陰影者怕重蹈覆轍」（persona P2）。

### 🟡 P1 / P2 — 次要優化
- [ ] 無責退回入口，從支援區再複製一個輕量入口到「承諾決策點」（聊聊 / 旅程末）。
- [ ] 成長相簿近況卡，點入可連到「和牠同型、現在可認養」的貓（社會證明導流到轉換）。
